...
from pystoi import stoi as StoiLoss
...
class AdvASRBrain(ASRBrain):
    ...
    def __init__(  # noqa: C901
        ...
        self.tokenizer = None
        self.stoi_loss = NegSTOILoss(hparams["sample_rate"]).cuda()
        ...
        * ``compute_forward()``
        ...
        if self.attacker is not None:
            if stage == sb.Stage.TEST:
                adv_wavs = self.attacker.perturb_and_log(batch, tokenizer=self.tokenizer, log=self.hparams.my_log_file)
            else:
    ...
    def evaluate_batch_adversarial(self, batch, stage, target=None):
        ...
        * ``compute_objectives()``
        ...
        with torch.no_grad():
            ...
            stoi = StoiLoss(orig.detach().cpu().numpy()[0], adv_wav.detach().cpu().numpy()[0], 16000)
            # stoi = self.stoi_loss(orig, adv_wav)
            loss = self.compute_objectives(
                predictions, batch_to_attack, stage=stage, adv=True, targeted=targeted, stoi=stoi
            ).detach()
            if targeted:
                targetloss = loss
        ...
        return advloss, targetloss, stoi
    ...
    def evaluate(
        ...
        if self.attacker is not None:
            self.attacker.on_evaluation_start(load_audio=load_audio)
        ...
        for i, batch in enumerate(tqdm(test_set, dynamic_ncols=True, disable=not progressbar)): 
            self.step += 1
            loss = self.evaluate_batch(batch, stage=sb.Stage.TEST)
            avg_test_loss = self.update_average(loss, avg_test_loss)

            if self.attacker is not None:
                adv_loss, adv_loss_target, stoi = self.evaluate_batch_adversarial(
                ...
                avg_adv_stoi = self.update_average(
                    torch.tensor(stoi), avg_adv_stoi)
    ...
    def fit_attacker(
        ...
        self.on_evaluate_start(max_key=max_key, min_key=min_key)
        if self.attacker is not None:
            self.attacker.on_evaluation_start(load_audio=True)
        self.on_stage_start(sb.Stage.TEST, epoch=None)
        self.modules.eval()
        avg_test_loss = 0.0
    ...
    def on_stage_end(
        self, stage, stage_loss, epoch, stage_adv_loss=None, stage_adv_loss_target=None, stage_adv_stoi=None
    ):
        ...
        if stage_adv_stoi is not None:
            stage_stats["adv stoi"] = stage_adv_stoi
        if stage == sb.Stage.TRAIN:
        ...
        if stage == sb.Stage.VALID:
            ...
            with open(self.hparams.adv_wer_file, "w") as adv_wer:
                self.adv_wer_metric.write_stats(adv_wer)

    def on_evaluate_start(self, max_key=None, min_key=None):
        """Run at the beginning of evaluation.
