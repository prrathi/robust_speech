...
#                 "save_audio_path must be provided in order to load audio files")
    ...
    def perturb_and_log(self, batch, target=None, tokenizer=None, log=None):
        """
        ...
        if self.load_audio:
            try:
                ...
                pass
        if adv_wav is None:
            # 'at_position', 'batchsize', 'id', 'pin_memory', 'sig', 'to', 'tokens', 'tokens_bos', 'tokens_eos', 'wrd'
            adv_wav = self.perturb(batch, tokenizer=tokenizer, log=log)
            if self.save_audio_path:
